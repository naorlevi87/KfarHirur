-- supabase/migrations/20260614030000_commons_recurrence_lookahead.sql
-- Generate runs with a 1-day LOOK-AHEAD: today's run + tomorrow's, so tomorrow can be reviewed and
-- adjusted (ad-hoc add, defer, skip) the day before. "Missed" marking moves to a separate pass keyed
-- to the *current* operational day, so a pre-generated tomorrow is never marked missed. Generation is
-- idempotent (skips a routine/day whose run already exists), so re-running is safe.
